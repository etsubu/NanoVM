...
func NewAssembler() *Assembler {
	...
	opcodeMap["mov"] = Opcode{opcode: 0, size: 2, Signed: true}
	opcodeMap["add"] = Opcode{opcode: 1, size: 2, Signed: true}
	opcodeMap["sub"] = Opcode{opcode: 2, size: 2, Signed: true}
	opcodeMap["and"] = Opcode{opcode: 3, size: 2, Signed: true}
	opcodeMap["or"] = Opcode{opcode: 4, size: 2, Signed: true}
	opcodeMap["xor"] = Opcode{opcode: 5, size: 2, Signed: true}
	opcodeMap["shr"] = Opcode{opcode: 6, size: 2, Signed: true}
	opcodeMap["shl"] = Opcode{opcode: 7, size: 2, Signed: true}
	opcodeMap["mul"] = Opcode{opcode: 8, size: 2, Signed: true}
	opcodeMap["div"] = Opcode{opcode: 9, size: 2, Signed: true}
	opcodeMap["mod"] = Opcode{opcode: 10, size: 2, Signed: true}
	opcodeMap["cmp"] = Opcode{opcode: 11, size: 2, Signed: true}

	opcodeMap["jz"] = Opcode{opcode: 12, size: 1, RelativeAddress: true, Signed: true}
	opcodeMap["jnz"] = Opcode{opcode: 13, size: 1, RelativeAddress: true, Signed: true}
	opcodeMap["jg"] = Opcode{opcode: 14, size: 1, RelativeAddress: true, Signed: true}
	opcodeMap["js"] = Opcode{opcode: 15, size: 1, RelativeAddress: true, Signed: true}
	opcodeMap["jmp"] = Opcode{opcode: 16, size: 1, RelativeAddress: true, Signed: true}
	opcodeMap["not"] = Opcode{opcode: 17, size: 1, Signed: true}
	opcodeMap["call"] = Opcode{opcode: 18, size: 1, RelativeAddress: true, Signed: true}
	opcodeMap["push"] = Opcode{opcode: 19, size: 1, Signed: true}
	opcodeMap["pop"] = Opcode{opcode: 20, size: 1, Signed: true}
	opcodeMap["ret"] = Opcode{opcode: 21, size: 0, Signed: true}
	opcodeMap["halt"] = Opcode{opcode: 22, size: 0, Signed: true}
	opcodeMap["load"] = Opcode{opcode: 23, size: 2, Signed: true}
	opcodeMap["load8"] = Opcode{opcode: 24, size: 2, Signed: true}
	opcodeMap["load8u"] = Opcode{opcode: 25, size: 2, Signed: true}
	opcodeMap["store"] = Opcode{opcode: 26, size: 2, Signed: true}
	opcodeMap["store8"] = Opcode{opcode: 27, size: 2, Signed: true}
	opcodeMap["store8u"] = Opcode{opcode: 27, size: 2, Signed: false}
	opcodeMap["syscall"] = Opcode{opcode: 28, size: 2, Signed: true}
	opcodeMap["printi"] = Opcode{opcode: 29, size: 1, Signed: true}
	opcodeMap["alloc"] = Opcode{opcode: 30, size: 2, Signed: true}
	opcodeMap["free"] = Opcode{opcode: 31, size: 1, Signed: true}
	opcodeMap["inc"] = Opcode{opcode: 32, size: 1, Signed: true}
	opcodeMap["dec"] = Opcode{opcode: 33, size: 1, Signed: true}
		...
		if reg, ok := a.RegisterMap[dstRegStr]; ok {
			...
			if instruction.opcode.size == 2 {
				if reg, ok = a.RegisterMap[sourceValue]; ok {
					...
					immediate, err := ConvertToNumber(sourceValue, instruction.opcode.Signed)
					if err != nil {
						return InstructionSet{}, &AssemblerError{lineNumber: i + 1, line: line, message: err.Error()}
					}
			...
			immediate, err := ConvertToNumber(dstRegStr, instruction.opcode.Signed)
			if err != nil {
				return InstructionSet{}, &AssemblerError{lineNumber: i + 1, line: line, message: err.Error()}
			}
			instruction.sourceType = Immediate
