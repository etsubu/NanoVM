...
func TestAssembleInstructionEncoding(t *testing.T) {
	tests := []struct {
		...
		{"mov reg0, reg1.32", []byte{0x00, 0x24}},
		{"mov reg0, reg1.64f", []byte{0x00, 0x2e}},
		...
		{"mov reg0, 1.5", []byte{0x00, 0x0f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf8, 0x3f}},
		{"store reg0, 123", []byte{0x1b, 0x01, 0x7b}},
		{"shr_s reg0, 3", []byte{0x1d, 0x01, 0x03}},
		{"jz reg0", []byte{0x0f, 0x00}},
		{"jmp -2", []byte{0x13, 0x01, 0xfe}},
		// Width is picked so the opcode's own extension rule reproduces the literal
		{"mov reg0, 255", []byte{0x00, 0x01, 0xff}},
		{"mov reg0, -1", []byte{0x00, 0x07, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}},
		{"div reg0, 255", []byte{0x09, 0x01, 0xff}},
		{"div_s reg0, -1", []byte{0x0a, 0x01, 0xff}},
		{"cmp_s reg0, 255", []byte{0x0e, 0x03, 0xff, 0x00}},
		{"jmp 200", []byte{0x13, 0x03, 0xc8, 0x00}},
...
func TestAssembleLabelResolution(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			"jz :end\nhalt\n:end\nhalt",
			[]byte{0x0f, 0x07, 0x0b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x19, 0x19},
		},
		{
			"backward relative jump",
			":top\nhalt\njmp :top",
			[]byte{0x19, 0x13, 0x07, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
		},
		{
			"label at end of file",
			"jz :end\nhalt\n:end",
			[]byte{0x0f, 0x07, 0x0b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x19},
		},
		{
			"two labels on the same instruction",
			"jz :a\njnz :b\n:a\n:b\nhalt",
			[]byte{
				0x0f, 0x07, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				0x10, 0x07, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				0x19,
			},
		},
		{
			...
			":target\nhalt\nmov reg0, :target",
			[]byte{0x19, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
		},
	}
	for _, tc := range tests {
...
func TestInstructionLengthMatchesAssembledSize(t *testing.T) {
	sources := []string{
		...
		"mov reg0, 100000", "mov reg0, 10000000000", "mov reg0, 1.5", "not reg0",
		"ret", "push reg0", "jmp -2", "shr_s reg0, 3", "mov reg0, -1",
	}
...
func TestRegisterMapContainsOnlyBaseAndSuffixedNames(t *testing.T) {
	...
	suffixes := []string{"", ".8", ".16", ".32", ".64", ".32f", ".64f"}
